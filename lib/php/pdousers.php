    ...
    if (!empty($_POST)) {
        ...
        if ($_POST['u']['passwd'] == $u['passwd']) {
            ...
            $m = 'Password does not match';
                ...
                $m = 'Account is disabled, contact your System Administrator';
            }
        } else {
                            ...
                            if (filter_var($_POST['u']['email'], FILTER_VALIDATE_EMAIL)) {
                                ...
                                if (is_admin()) {
                                    ...
                                    if (mail_activate($_POST['u']['uid'], $_POST['u']['email'], $pwkey)) {
                                        ...
                                        $m = 'Problem sending message to '.$_POST['u']['email'];
                                    }
                                }
                            } else {
                                $m = 'You must provide a valid email address';
                            }
                        } else {
                            $m = 'Email address already exists';
                        }
                    } else {
                        $m = 'Username already exists';
                    }
                } else {
                    $m = 'Username must be only a-z, A-Z, 0-9';
                }
            } else {
...
function changepw()
{
    ...
    if (!empty($_POST)) {
        ...
        if (strlen($_POST['u']['passwd']) > 7) {
            ...
            $m = 'New password must be at least 8 characters';
        }
        $_SESSION['m'] = $m;
    }
...
function activate()
{
  ...
  $key = isset($_GET['key']) ? $_GET['key'] : 'dudkey';
    ...
    if ($id) {
        ...
        $_SESSION['m'] = 'Incorrect activation, please try again';
    }
    header('Location: '.$_SERVER['PHP_SELF']);
    exit();
}
...
function install()
{
 ...
 CREATE TABLE `'.$tbl."` (
    ...
    `created` datetime
        ...
        $_SESSION['m'] = 'Database table already exists';
    }
    header('Location: '.$_SERVER['PHP_SELF']);
    exit();
}
