...
error_log(__METHOD__);
    ...
    if (!msg()) $this->buf = $this->table();
  }

  function pager_session($db)
  {
error_log(__METHOD__);
...
error_log(__METHOD__);
  ...
  WHERE {$this->c} LIKE '{$pre}{$find}{$post}'";
    ...
    if ($this->w) $where = $where ? $where.$this->w : "
  WHERE ".$this->w;
...
error_log("sql = $sql");
    ...
    try {
      if ($where) $this->totalrows = (int)$this->dbh->query("
...
//FIXME: $this->startrows needs to be reset before query()
  ...
  function pager($startpage, $endpage, $numpages)
  {
...
error_log(__METHOD__);
    ...
    $buf = '';
    foreach($this->columns as $k=>$v) $buf .= '
                      <th>'.$this->column($v).'</th>';
    return '
                    <tr>'.$buf.'
                    </tr>';
